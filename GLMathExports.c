...
vec2_t _vec2_create(float x, float y) { return vec2_create(x, y); }
...
float _vec2_dot(const vec2_t v1, const vec2_t v2) { return vec2_dot(v1, v2); }
float _vec2_magSquared(const vec2_t v) { return vec2_magSquared(v); }
float _vec2_mag(const vec2_t v) { return vec2_mag(v); }
...
vec3_t _vec3_create(float x, float y, float z) { return vec3_create(x, y, z); }
...
float _vec3_dot(const vec3_t v1, const vec3_t v2) { return vec3_dot(v1, v2); }
float _vec3_magSquared(const vec3_t v) { return vec3_magSquared(v); }
float _vec3_mag(const vec3_t v) { return vec3_mag(v); }
...
vec4_t _vec4_create(float x, float y, float z, float w) { return vec4_create(x, y, z, w); }
...
float _vec4_dot(const vec4_t v1, const vec4_t v2) { return vec4_dot(v1, v2); }
float _vec4_magSquared(const vec4_t v) { return vec4_magSquared(v); }
float _vec4_mag(const vec4_t v) { return vec4_mag(v); }
...
float _mat3_det(const mat3_t m) { return mat3_det(m); }
...
float _mat4_det(mat4_t m) { return mat4_det(m); }

#pragma mark - Quaternions

quat_t _quat_createf(float x, float y, float z, float angle) { return quat_createf(x, y, z, angle); }
quat_t _quat_createv(vec3_t axis, float angle) { return quat_createv(axis, angle); }
...
float _quat_magSquared(const quat_t q) { return quat_magSquared(q); }
float _quat_mag(const quat_t q) { return quat_mag(q); }
...
float _quat_dotProduct(const quat_t qA, const quat_t qB) { return quat_dotProduct(qA, qB); }
quat_t _quat_slerp(const quat_t qA, const quat_t qB, float t) { return quat_slerp(qA, qB, t); }
...
vec3_t _bezier_getPoint(bezier_t curve, float t) { return bezier_getPoint(curve, t); }
float _bezier_getCoordForAxis(bezier_t curve, float t, bezierAxis_t axis) { return bezier_getCoordForAxis(curve, t, axis); }
vec3_t _bezier_firstDerivative(bezier_t curve, float t) { return bezier_firstDerivative(curve, t); }
...
vec3_t _bezier_getPointWithOffset(bezier_t curve, float t, vec3_t offset) { return bezier_getPointWithOffset(curve, t, offset); }
void _bezier_getLineSegments(bezier_t curve, int count, vec3_t *outPoints, float *outLengths, float *outDeltas, float *outTotalLength) { bezier_getLineSegments(curve, count, outPoints, outLengths, outDeltas, outTotalLength); }
vec3_t _bezier_getPointUsingLineSegments(float t, int count, vec3_t *points, float *lengths, float *deltas, float totalLength) { return bezier_getPointUsingLineSegments(t, count, points, lengths, deltas, totalLength); }

#pragma mark - Rectangles

rect_t _rect_createWithSize(vec2_t origin, vec2_t size) { return rect_createWithSize(origin, size); }
rect_t _rect_createWithCorners(float left, float bottom, float right, float top) { return rect_createWithCorners(left, bottom, right, top); }
float _rect_maxX(const rect_t rect) { return rect_maxX(rect); }
float _rect_minX(const rect_t rect) { return rect_minX(rect); }
float _rect_maxY(const rect_t rect) { return rect_maxY(rect); }
float _rect_minY(const rect_t rect) { return rect_minY(rect); }
...
float _rect_area(const rect_t rect) { return rect_area(rect); }
...
mat4_t _mat4_create_translation(float x, float y, float z) { return mat4_create_translation(x, y, z); }
mat4_t _mat4_translate(mat4_t mat, float x, float y, float z) { return mat4_translate(mat, x, y, z); }
mat4_t _mat4_create_rotation(float angle, float x, float y, float z) { return mat4_create_rotation(angle, x, y, z); }
mat4_t _mat4_rotate(mat4_t mat, float angle, float x, float y, float z) { return mat4_rotate(mat, angle, x, y, z); }
mat4_t _mat4_create_scale(float x, float y, float z) { return mat4_create_scale(x, y, z); }
mat4_t _mat4_scale(mat4_t mat, float x, float y, float z) { return mat4_scale(mat, x, y, z); }
...
void _matrix_stack_translate(matrix_stack_t *stack, float x, float y, float z) { matrix_stack_translate(stack, x, y, z); }
void _matrix_stack_rotate(matrix_stack_t *stack, float angle, float x, float y, float z) { matrix_stack_rotate(stack, angle, x, y, z); }
void _matrix_stack_scale(matrix_stack_t *stack, float x, float y, float z) { matrix_stack_scale(stack, x, y, z); }
